...
define("IN_MYBB", 1);
define('THIS_SCRIPT', 'credit.php');
define("NP_DISABLE_GUESTS", 0);

// Templates used by NewPoints
$templatelist  = "newpoints_home,newpoints_transfer,newpoints_statistics,newpoints_statistics_richest_user,newpoints_statistics_donation,newpoints_no_results,newpoints_option";
...
$menu[0] = '<a href="'.$mybb->settings['bburl'].'/credit.php">'.$lang->newpoints_home.'</a>';
if ($mybb->settings['newpoints_main_statsvisible'] == 1)
	$menu[1] = '<a href="'.$mybb->settings['bburl'].'/credit.php?action=stats">'.$lang->newpoints_statistics.'</a>';
if ($mybb->settings['newpoints_main_donationsenabled'] == 1)
	$menu[2] = '<a href="'.$mybb->settings['bburl'].'/credit.php?action=transfer">'.$lang->newpoints_transfer.'</a>';
...
elseif ($mybb->input['action'] == 'transfer')
{
	$menu[2] = "&raquo; ".$menu[2];
}
...
elseif ($mybb->input['action'] == 'do_donate')
{
	...
	redirect($mybb->settings['bburl']."/credit.php", $lang->sprintf($lang->newpoints_donated, newpoints_format_points($amount)));
}
