<?php
/* by Tomasz 'Devilshakerz' Mlynski [devilshakerz.com]; Copyright (C) 2014-2021
...
always=Always check if on display to refresh',
        ...
        [
            ...
            'description' => 'Choose whether Shoutbox window should be expanded (On) or collapsed (Off) by default.',
            'optionscode' => 'onoff',
            'value'       => '1',
        ],
        [
...
{
    global $mybb;
    return isset($mybb->settings['dvz_sb_num']);
}
...
dvz_shoutbox.init();
    ...
    static function show_archive()
    {
        ...
        if ($mybb->get_input('unmark_all') && verify_post_check($mybb->get_input('postkey'))) {
            ...
            header('Location: index.php?action=shoutbox_archive');
            exit;
        }
        ...
        $lastReadId = $mybb->get_input('last_read', MyBB::INPUT_INT);

        if ($lastReadId) {
            $resolvedEntry = self::get($lastReadId, true);

            if ($resolvedEntry) {
                $location = 'index.php?action=shoutbox_archive&sid=' . (int)$resolvedEntry['id'] . '#sid' . (int)$resolvedEntry['id'];
            } else {
                $location = 'index.php?action=shoutbox_archive&page=' . (int)$pages;
            }
        ...
        if (
            ...
            $page == 1 &&
            $lastId !== null &&
            $lastId != abs((int)$mybb->cookies['dvz_sb_last_read'])
        ) {

            $sid = abs((int)$mybb->cookies['dvz_sb_last_read']);
            $last_read_url = $mybb->settings['bburl'] . '/index.php?action=shoutbox_archive&last_read=' . $sid;
...
dvz_shoutbox.lang = [\'' . $lang->dvz_sb_delete_confirm . '\', \'' . str_replace('{ANTIFLOOD}', (float)$mybb->settings['dvz_sb_antiflood'], $lang->dvz_sb_antiflood) . '\', \'' . $lang->dvz_sb_permissions . '\'];
    ...
    // data handling
    static function get($id, $orLower = false)
    {
        global $db;

        if ($orLower) {
            $where = 'id<=' . (int)$id . ' AND s.text IS NOT NULL ORDER BY id DESC LIMIT 1';
        } else {
            $where = 'id=' . (int)$id . ' AND s.text IS NOT NULL';
        }

        return $db->fetch_array(
            $db->simple_select('dvz_shoutbox s', '*', $where)
        );
    }
    ...
    static function access_mod()
    {
        ...
        return (
            (in_array(-1, $array) || is_member($array)) ||
    ...
    static function access_mod_own()
    {
        ...
        return in_array(-1, $array) || is_member($array);
    }
